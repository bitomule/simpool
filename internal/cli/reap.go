...
func RunReap(args []string, stdout, stderr io.Writer) int {
	...
	warmCap := fs.Int("warm", 0, "how many free simulators to keep warm per device+OS group, independent of --max (which caps how many may be resident/locked at once, not how many stay booted afterward). Both a ceiling and a floor: beyond N the most-recently-used are kept and the rest shut down regardless of --cold, and below N free+shut-down slots are BOOTED to reach it, most-recently-used first — measured, provisioning a booted slot takes ~6s against ~18.5s for a cold one, and `preboot` exists to pay that ahead of time but nothing on this machine invokes it. The floor never boots while free memory is under 35%, and says so rather than declining quietly. 0 (default) disables both halves and preserves today's behavior, where only --cold's idle-time check ever shuts a free slot down")
	...
	if *warmCap > 0 {
		for _, groupDir := range groups {
			// Ceiling first, then floor, and the order is not arbitrary:
			// the ceiling pass may shut a simulator down, which changes how
			// many are warm, so a floor computed before it would boot a
			// slot the next line turns off again. They can never contend
			// for the same slot anyway — one acts only on Booted devices
			// and the other only on Shutdown ones.
			enforceWarmCap(root, groupDir, *warmCap, *dryRun, stdout, stderr)
			enforceWarmFloor(root, groupDir, *warmCap, *dryRun, stdout, stderr)
		}
	}
