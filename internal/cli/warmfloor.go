...
func warmFloorThreshold() float64 {
	raw := os.Getenv(EnvWarmMinFree)
	if raw == "" {
		return warmFloorMinFreeMemory
	}
	// An INTEGER percentage, deliberately: "0.35" is a valid float in range
	// and would mean 0.35%, which all but disables the guard while reading
	// like someone carefully setting 35%. Refusing it is the difference
	// between a typo that falls back to the default and a typo that quietly
	// removes the protection. Found by the test for this function.
...
//
// A package-level var so tests can drive both sides of the threshold
...
// machine as nearly full and this would never warm anything.
