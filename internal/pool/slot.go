...
func tryAcquireSlots(root, device, osVersion string, count, max int) ([]*Slot, error) {
	...
	take := func(n int) (bool, error) {
		...
		if poison := CheckPoison(meta); poison.Poisoned() {
			...
			// window) and its consumer is still running even though the
			...
			// (start time + machine boot fingerprint) still matches reality
			// (see AttemptRecovery); never based on LiveConsumers alone.
			if !AttemptRecovery(dir, &meta, poison) {
				// Couldn't verify identity (or the kill didn't stick) —
				// quarantine exactly as before this feature existed and
				// skip to the next candidate slot.
				lock.Release()
				return false, nil
			}
		}
